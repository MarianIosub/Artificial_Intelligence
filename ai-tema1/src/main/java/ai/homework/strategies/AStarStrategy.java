...
public class AStarStrategy extends Strategy {
    ...
    private Map<State, Integer> adjacentStates = new HashMap<>();

    private List<State> visited = new ArrayList<>();

    public AStarStrategy(int numberOfCouples) {
        super(numberOfCouples);
    }

    /**
     * @return Distanta de la nodul initial la nodul actual.
     */
    private int computeDistanceFromInitialNodeToActualNode() {
                ...
                for (int femaleIndex2 = -1; femaleIndex2 < numberOfCouples; femaleIndex2++) {
                    for (int maleIndex2 = -1; maleIndex2 < numberOfCouples; maleIndex2++) {
                        ...
                        if (adjacentState != null) {
                            ...
                            // Distanta de la nodul initial la nodul adiacent
                            stateEuristic += computeDistanceFromInitialNodeToActualNode() + 1;

                            adjacentState.setPreviousState(actualState);
